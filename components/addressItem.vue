<template>
    <div class="bgfff pl9 pr18 disflex pt20">
        ...
        <div class="disflex jsbet flex1">
            <div class="addr_detail" v-if="addr.name">
                <p class="fs16 c38 fbold">
                    <span>{{addr.name}}</span>
                    <span style="margin-left: 20rpx"> {{addr.tel}}</span>
                </p>
                <!-- over_1 -->
                <p class="fs13 c38  pt10 pb21">{{addr.address}}</p>
            </div>
            <div class="addr_detail pb21 fs16 c38" v-else>去添加地址</div>

            <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/right.png"
                 alt=""
                 class="w10 h10 mt14"
                 v-if="ismore"
                 @click="toAddr">
