<template>
  <div class="fix_top h100p dialog_box">
    <div class="dialog_con bgfff bradius5" >
      ...
      <div class="pl25 pr25" v-if="type != 'hint1' && type != 'hint2'">
        <input type="text" maxlength='-1' class="pha8 bradius5" :placeholder="dialog_ph" v-model="text">
        <input type="text" maxlength='-1' class="pha8 bradius5" :placeholder="dialog_ph_2" v-model="text_2" v-if="type == 'input_2'">
      </div>

      <div v-if="type == 'hint1'" class="fs14 c78 pl35 pr31 pb36">
        <template v-if="memberType == 1">
          该用户是本机构成员，不能添加为客户
        </template>
        <template v-if="memberType == 2">
          该客户已被成员<span class="cblue">{{memberInfo}}</span>独占，请与他取得联系
        </template>
        <template v-if="memberType == 3">
          <span class="cblue">共享客户池</span>已存在该客户，前去抢为独占
        </template>
...
<script>
  ...
  export default {
    name: '',
    props:['dialog_title','dialog_ph','dialog_ph_2','default','default_2','left' ,'right','type', 'memberInfo', 'memberType'],
    components: {},
    data() {
      return{
        text:'',
