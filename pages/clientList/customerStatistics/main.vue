   ...
   :style="'background-position: 0 ' + navHeight + 'px'">
    ...
    <div v-if="hasenterprise" class="mt10">
      ...
      <div v-show="menu_id == 0 || menu_id == 2">
        <customerData
          :isChange="isChange"
          :customerInteract="customerInteract"
          :customerInteractMax="customerInteractMax"
...
<script>
import customerData from "@/components/customerData"; // 个人总览
...
export default {
  ...
  components: { customerData, SalesRanking, StaticResources, Notice , statusbarTitle},
  data() {
    return {
  ...
  methods: {
    ...
    toDetall(type){
      switch(type){
        case 'customer':
        wx.navigateTo({
          url:"../companyPack/salesRanking/main"
        })
        break;
        case 'sale':
          wx.navigateTo({
