<template>
  <div class="h100p posre overscroll">
    <!--top-->
    <div class="mt10">
      <MsgBox :msg="msgitem"
              :istel="true"
    ...
    <div class="bgfff lh49 fs16 c38  pl15 pr15 bbf5f6">

      <div class="disflex jsbet">
        <span class="fbold pr10">备注:{{msgitem.customerRemark  || '暂无备注'}} </span>
      ...
      <div class="bgfff disflex fs12 pt14 pb15 pl14 nowrap">
        <div class="shadow_gray textc pt15 pl15 pr15 borderbox h65 mr10" @click="makePhone">
          <p class="disflex align-cen fs12">
            <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/tel_2.png" alt="" class="w16 h16"/>
        ...
        <div class="shadow_gray textc pt15 pl15 pr15 borderbox h65 mr10" @click="addWx">
          <p class="disflex align-cen fs12">
            <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/im_msg.png" alt="" class="w16 h16" />
        ...
        <div class="shadow_gray textc pt15 pl15 pr15 borderbox h65 mr10" @click="copyText(msgitem.email || '')">
          <p class="disflex align-cen fs12">
            <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/email.png" alt="" class="w16 h16" />
        ...
        <div class="pr10">
          <div class="shadow_gray  pt15 pl15 pr15 borderbox h65 mr10" @click="copyText(msgitem.createCompanyName|| '')">
            <p class="disflex align-cen fs12">
              <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/company_3.png" alt=" " class="w16 h16" />
              <span class="pl9">机构</span>
            </p>
        ...
        <div class="pr10">
          <div class="shadow_gray  pt15 pl15 pr15 borderbox h65 mr10" @click="createRecord">
            <p class="disflex align-cen fs12">
              <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/company_3.png" alt=" " class="w16 h16" />
              <span class="pl9">添加拜访</span>
            </p>
            <!-- <p class="c78 fbold pt10 minw50" style="white-space: nowrap">{{visitTimes}}次</p> -->
          </div>
        </div>
    ...
    <!-- <div class="bgfff mb10 pb23">
      ...
      <p class="disflex align-cen jscen fs14 cb8" @click="page_turn('followLog','?cardId='+cardId)">
        ...
        <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/right.png" alt="" class="w10 h10 "/>
      </p>
    </div> -->


    <div class="bgfff lh49 fs16 c38 mb10 pl15 pr15">

      <div class="disflex jsbet" @click="editTab=true">
        <span class="fbold pr10">客户标签</span>
        <div class="disflex row-reverse align-cen">

          <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/right.png" alt="" class="w10 h10"/>
          <p class="fs12 ml10 ca8 mr4">编辑标签</p>
        </div>
      </div>
      <div class="disflex pt12 wrap pb7 pr15">
        <span class="ca8 textc pl12 pr13 lh25 fs14 bradius3 disblock bgf5f6 mr10"
              v-for="(v,k) in msgitem.lableResponseList" :key="k"
              :style="{ color : v.lableColor == '#F5F5F6' ? '#a8a8a8' : '#fff' ,
              background : v.lableColor } ">
              {{v.lableName ? v.lableName : ""}}
        </span>
      </div>
      <div class="disflex jsbet">
        ...
        <radio-group class="disflex">
          <label class="uni-list-cell mr10 uni-list-cell-pd" v-for="(item, index) in sharedScopes" :key="item.id">
            <radio :value="item.id" color="#51CDCB" :disabled="true" v-if="item.isSelect" :checked="index === share_scope.id"/>{{item.isSelect ? item.name : ''}}
          </label>
        </radio-group>
      </div>
      ...
      <div class=" pl42 pr43">
        ...
        <div class="disflex align-cen jscen" @click="logs_open_tap" v-if="tradingVolumeLists.length > 0">
          ...
          <span v-show="!logs_show">
            <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/down.png" alt=""  class="w10 h10 ml7"/>
          </span>
          <span v-show="logs_show">
            <img src="https://one-stand-maitan-hangzhou.oss-cn-hangzhou.aliyuncs.com/yimai_photos/crm/up.png" alt=""  class="w10 h10 ml7" />
          </span>
        </div>
      </div> -->


      <!--记录 v-if="tradingVolumeLists.length > 0"-->
    ...
    <div v-if="isShowCourse">
      ...
      <div class="posticky bgfff zindex9" style="top:0;">
        <div class="client-tab">
          <p
          v-for="(item,index) in clienttabs"
          @click="tab_tap(item.type,index)"
          :key="index"
          :class="current == index ? 'active' : ''">
            {{item.name}}
            <span :class="current == index ? 'isSelect' : '' "></span>
          </p>
        </div>
      </div>
       <ClientTabbar
        :logs="followLogs"
        :deallogs="tradingVolumeLists"
        :msg="msgitem"
        ref="tab"
        :current="current"
        :moneyLog="msgitem.turnoverAmount"
        :userId="userId"
        :cardId="cardId"
        :firstLog="firstLog"
        >
...
<script>
  ...
  export default {
    name: '',
    components: {BottomButtonTwo,DialogBox,MsgBox,Log, ClientAddModal, SelectorOne, AddWxCourse, StaticResources,ClientTabbar},
    data() {
      return{
        msgitem:  { },
        tel : '' ,
        ...
        isShowCourse: false,
        choose_tab_disabled:true,
        firstLog: '', // 第一条记录
        // typeLogs:'' //tabbar组件参数容器
    ...
    onLoad(){
      ...
      this.$refs.tab.getCardMsg(this.userId)
    },
    onShow(){
    ...
    methods: {
      ...mapMutations(['setSelectTab']),
      createRecord() {
        this.customerList = [this.msgitem.cardId, this.msgitem.name, this.msgitem.phone, this.msgitem.companyName, this.msgitem.position
        ];
      	wx.setStorageSync("customerList", this.customerList)
      	// console.log(this.customerList)
      	// this.customerList = [id,name,phone,companyName,position]
      	wx.navigateTo({
      		url: "/pages/companyPack/addVisit/main?"
      	});
      },
      tab_tap(type,index){
        if(index == 3){
          this.setSelectTab(true)
        }
        this.current = index
        console.log(index)
        if(index == 2){
      ...
      getClientMsg(){//获取客户信息
        ...
        WXAJAX.POST({
          ...
          if(data){
            ...
            this.firstLog = data.createTime+ '由 '+ ( data.followName ? data.followName : "")+' 主动加入';
            this.sharedScopes[0].isSelect = false
            this.sharedScopes[1].isSelect = false
            switch (data.customerState) {
              case 1 :
                this.$set(this.share_scope , 'name' ,'独占' ) ;
                this.sharedcurrent = 1
                this.share_scope.id = 1
                this.sharedScopes[1].isSelect = true
                break ;
              case 2 :
                this.$set(this.share_scope , 'name' ,'共享' ) ;
                this.sharedcurrent = 0
                this.share_scope.id = 0
                this.sharedScopes[0].isSelect = true
                break ;
              case 3 :
      ...
      page_turn(method , query){
        query = query || '' ;
        if(method == 'edit'){//录入成交金额
          // this.editTab = true ;
         wx.navigateTo({
          url: `/pages/clientList/inputMoney/main?customerId=${this.msgitem.customerId}`,
         });
        }else{
          if(method == 'oneself'){
      ...
      modal_btn_tap(method){//点击标签弹框按钮
        ...
        if(method == 'reset'){//重置
          ...
          this.choose_tab = [{},{},{}];
          this.choose_tab_disabled = false
      ...
      edit_tap(){
        wx.navigateTo({url:'../../companyPack/clientAdd/main?type=edit&cardId='+this.cardId})
      },
      ...
      //   switch(type){
      //     case 'track' :
      //     this.typeLogs = this.followLogs
      //     break;
      //     case 'deal' :
      //     this.typeLogs = this.tradingVolumeLists
      //     break;
      //   }
      // }
    }
