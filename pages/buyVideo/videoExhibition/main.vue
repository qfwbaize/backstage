        ...
        <div v-for="(videoItem, index) in videoLists" :key="index">
            ...
            <div class="bgfff textc borderbox h212 overhidden">
                ...
                <div class="w100p h212 posre" @click="showVideo(videoItem.videoId)" v-else>
                    <img mode="aspectFill" :src="videoItem.videoCover || 'https://myy-one-stand.oss-cn-beijing.aliyuncs.com//one-www/photo/20190604/1559619324240.png'" alt="" class="w100p h212">
                    <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com//one-www/photo/20190604/1559619365051.png" alt="" class="w50 h50 posab top0 left0 right0 bottom0 mauto">
                </div>
            </div>
            <div class="disflex jsbet bffff mt20 pb20 pl15 pr15 h30 overhidden">
                <div class="w30 h30">
                    <img @click="deleteVideo(videoItem, index)" src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/editor_del_2.png" alt="" class="w30 h30">
                </div>
                <div class="w70 h30 disflex jsbet">
                    <img @click="moveVideo(videoItem, index, '1')" src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/editor_up.png" alt="" class="w30 h30">
                    <img @click="moveVideo(videoItem, index, '2')" src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/editor_down.png" alt="" class="w30 h30">
                </div>
            </div>
        </div>
        <BottomButtonSmall :text="'新增视频'" :url="'save'" @btn_tap="btn_tap"></BottomButtonSmall>
...
export default {
    ...
    onShow() {
      ...
      WXAJAX.POST({ignore: true},'', '/businessCardVideo/moveList').then((data, code) => {
        wx.hideLoading();
    ...
    methods: {
        ...
        toEditVideo(editExhibition){
            ...
            wx.navigateTo({
                url: '../buyVideo/editVideoExhibition/main?type=edit'
            })
        },
        // 新增视频
        btn_tap(){
        ...
        deleteVideo(videoItem, index){
            wx.showLoading();
            WXAJAX.POST({videoId: videoItem.videoId}, '', '/businessCardVideo/delVideo').then((data) => {
              wx.hideLoading();
              wx.showToast({
                  title: '删除成功',
                  duration: 2000,
        ...
        moveVideo(videoItem, index, type){
            // type=1 为上移; 为2是下移
            let videoTemp = []
            this.videoLists.forEach(item => {videoTemp.push(item)})
            if (type == 1) {
              if (index == 0) {
                wx.showToast({
                  title: '不能再上移了噢' ,
            ...
            wx.showLoading();
            WXAJAX.POST({videoId: videoItem.videoId, sorts: temp}, '', '/businessCardVideo/moveVideo').then((data) => {
                this.videoLists = []
                wx.hideLoading();
                wx.showToast({
                  title: '移动成功',
