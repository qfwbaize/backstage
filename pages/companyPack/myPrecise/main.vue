  ...
  <div class="item ">
    ...
    <scroll-view :style="{height: scrollContentHeight+'px'}"
        ...
        <div class="item-box bgfff" v-for="info in shareData" :key="info.dynamicId">
          <div @click="toDetail(info.dynamicId,info.shareId,info.preciseId)" class="middle flex-base pl15 pr15">
            <img :src="info.photos"
                 alt=""
                 mode="aspectFill"
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if(this.status==2){
          data = await WXAJAX.POST(
            ...
            "",
            "/precise/record/list/preciseDynamic",'','',1
          );
        }else{
          data = await WXAJAX.POST(
            { pageNum:page,pageSize:10},
            "",
            "/precise/record/list/preciseMyDynamic",'','',1
          );
        }
        if (data) {
    ...
    toDetail(dynamicId,shareId,preciseId) {//查看文章详情
      let type=1
      if(this.status!=2){
        type=2;
      }
      let url = '../../dynamicDetail/main?dynamicId=' + dynamicId +'&insideShareId=' + shareId +'&type=' + type + '&preciseId=' + preciseId;
