  ...
  <div>
    ...
    <div class="disflex jsbet bgfff lh30 pl16 pr15 pt5 pb5">
      <div class="bgf5f6 bradius17 disflex pl15 flex1">
        ...
        <span></span>
        <input type="text" v-model="key" @focus="onFocus" @change="changedFn" class="pha8 fs14 lh34 h34 w100p" :placeholder="ph">

      </div>
      <span class="fs16 cblue lh34 pl15" @click="over">完成</span>
    ...
    <div v-if="type=='company' && showCreateCopmany" class="create-company bgfff">
      <div class="pt80 textc">
        <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com//one-www/photo/20190520/1558319783804.png" alt="" class="w170 h170">
        <div class="create-company-prompt fs12 lh32 ca8">请在上方搜索机构加入</div>
        <div class="create-company-prompt fs12 ca8">或返回选择我是管理者创建机构</div>
<!--        <div class="create-company-btn bradius5 cfff textc lh44 h44 w290 ml50p mt10" @click="toCreateCompany">创建机构</div>-->
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    onShow(option) {
      ...
      if (this.type == 'company') {//选择机构
        title = '选择机构';
        this.ph = '输入机构名称';
        this.activeitem = '当前机构';
        this.reset = '创建机构';
        this.init();
    ...
    methods: {
      ...
      toCreateCompany(){
        wx.navigateTo({
          url: `/pages/companyPack/editEnterprise/main?type=add&backNum=2`, // 跳转的目标页面
          success(res) {
          },
          fail(res) {
            console.log(res)
      ...
      init() {//获取lists列表
        ...
        v.lists = [];
        if(this.type == 'client'){
          WXAJAX.POST({ignore: true}, '', url,'1').then((data) => {
            this.lists = data ;
          }).catch((err) => {
      ...
      searchItem() {//搜索
        ...
        }, '', '/company/getCompanyAllInfo','1').then((data) => {
