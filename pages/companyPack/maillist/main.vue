		...
		<scroll-view :style="{height: scrollContentHeight+'px'}" class="dynamic-content" :scroll-y="true"
			...
			<div class="bgfff pt15 pl16 pr15">
				...
				<div class=" my-card-case flex-sb-c pb15">
					<p class="flex-c-c"> <span class="separator"></span> <span class="fs16 pl9">我的名片夹</span></p>
					<div class="right flex-sb-c">
						<span class="fs16 cblue pr8" v-if="lists.length>0">{{lists.length}}</span>
						<img class="card-case-num-img "
							src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/card-case-num.png"
							alt="">
					</div>
				</div>
				...
				<div v-if="lists.length>0">
					<div class="bradius10" v-for="(v,k) in lists" :key="k">
						<div class="bgfff bradius10">
							<BusinessCard :card_msg="v" :type="'plus'" :hasCard="false" :isdel="true" :index="k+1"
								:isLogin="isLogin" @needLogin="needLogin" @moreTap="moreTap" @shareApp="shareApp">
							</BusinessCard>
						</div>
					</div>
				</div>
			...
			<div class="bgfff pt15 pl16 pr15">
				<!--我的同事-->
				<div class=" my-card-case flex-sb-c pb15">
					<p class="flex-c-c"> <span class="separator"></span> <span class="fs16 pl9">我的同事</span></p>
					<div class="right flex-sb-c">
						<span class="fs16 cblue pr8" v-if="colleague_lists.length>0">{{colleague_lists.length}}</span>
						<img class="card-case-num-img "
							src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/card-case-num.png"
							alt="">
					</div>
				...
				<div v-if="colleague_lists.length>0">
					<div class="bradius10" v-for="(v,k) in colleague_lists" :key="k">
						<div class="bgfff bradius10">
							<BusinessCard :card_msg="v" :type="'plus'" :hasCard="false" :isdel="true" :index="k+1"
								:isLogin="isLogin" @needLogin="needLogin" @moreTap="moreTap" @shareApp="shareApp">
							</BusinessCard>
			...
			<div class="bgfff pt15 pl16 pr15">
				<!--我的客户-->
				<div class=" my-card-case flex-sb-c pb15">
					<p class="flex-c-c"> <span class="separator"></span> <span class="fs16 pl9">我的客户</span></p>
					<div class="right flex-sb-c">
						<span class="fs16 cblue pr8" v-if="customer_lists.length>0">{{customer_lists.length}}</span>
						<img class="card-case-num-img "
							src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/card-case-num.png"
							alt="">
					</div>
				</div>
				<div v-if="customer_lists.length>0">
					<div class="bradius10" v-for="(v,k) in customer_lists" :key="k">
						<div class="bgfff bradius10">
							<BusinessCard :card_msg="v" :type="'plus'" :hasCard="false" :isdel="true" :index="k+1"
								:isLogin="isLogin" @needLogin="needLogin" @moreTap="moreTap" @shareApp="shareApp">
							</BusinessCard>
			...
			<SelectorOne v-if="operateShow == true" :title="'操作'" :status="operateShow" :allClass="operateTypes"
				@closeModal="operateShow = !operateShow" @choose_tap="choose_tap"></SelectorOne>
...
<script>
	...
	export default {
		...
		onShareAppMessage(e) {
			...
			let imgurl = e.target.dataset.logo ||
				'https://thirdwx.qlogo.cn/mmopen/vi_32/VTFUkCPPwZdpB21d9dtLXCdRra3OyEboJvo0KGcczrnGTX7uKhsyTOwCywAx3F2xicrGb8wYq8H9VpYLR0W5e8A/132'
			...
			let {
				sharespeech = '', position, name
			} = e.target.dataset;
			sharespeech = sharespeech.replace('#name#', name).replace('#company#', companyName);
			return {
				title: sharespeech || `您好，我是${companyName}${position || ''}的${name}，敬请惠存!`,
		...
		onShow() {
			...
			this.getAllCard();
			this.getColleague_lists();
			this.getCustomer_lists();
		},
		...
		methods: {
			...
			getNotice() {
				WXAJAX.POST({}, '', '/notice/selectEffectiveList', '', '', 2).then(res => {
					this.noticeList = res || [];
				});
			},
			...
			getColleague_lists() {
				...
				WXAJAX.POST({
					...
					if (v.page === 1) {
						this.colleague_lists = datas;
					} else {
						this.colleague_lists.push(...datas);
					}
					console.log("colleague_lists2", this.colleague_lists)
				}).catch((err) => {});
			},
			getCustomer_lists() {
				let v = this;
				WXAJAX.POST({
					...
					data.forEach(function(i, k) {
						const cardData = {
							picchecked: i.userLogo || '',
							username: i.name || '',
							tel: i.phone || '',
							post: i.position || '',
							company: i.customerCompanyName || '',
							company_logo: i.companyLogo || '',
							cardId: i.cardId || '',
						};
						cardData.createTime = util.getdate(i.createTime, 'dateTime');
					...
					if (v.page === 1) {
						this.customer_lists = datas;
					} else {
						this.customer_lists.push(...datas);
					}
					console.log("customer_lists", this.customer_lists)
				}).catch((err) => {});
			},
