  ...
  <div class="item">
    ...
    <scroll-view :style="{height: scrollContentHeight+'px'}"
        ...
        <div class="item-box bgfff" v-for="info in shareData" :key="info.dynamicId">
          <div @click="toDetail(info.dynamicId,info.shareId,info.pushEffectId)" class="middle flex-base pl15 pr15">
            <img :src="info.photos"
                 alt=""
                 mode="aspectFill"
  ...
  methods: {
    ...
    async getqueryDynamicData(type,queryType,page) {
      ...
      try {
        ...
        //   "",
        //   "/pushEffect/record/list/pushEffectDynamic",'','',1
        // );
        let data = '';
        if(this.status==2){
          ...
          data = await WXAJAX.POST(
            { pageNum:page,pageSize:10},
            "",
            "/pushEffect/record/list/pushEffectMyDynamic",'','',1
          );
        }
        if (data) {
    ...
    toDetail(dynamicId,shareId,pushEffectId) {//查看文章详情
      let type=1
      if(this.status!=2){
        type=2;
      }
      let url = '../../article/dynamicDetail/main?dynamicId=' + dynamicId +'&insideShareId=' + shareId +'&type=' + type + '&pushEffectId=' + pushEffectId
