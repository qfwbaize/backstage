      ...
      <div v-for="(v,k) in classify" :key="k" class="bgfff ">
        <div class="disflex jsbet pr11" >
                    <span class="lh44 pl15 fbold"
                          @click="manage('edit',v.productsTypeName , v.productsTypeId)">
                        {{v.productsTypeName}}
                        <span v-if="v.goodsModelList">({{v.goodsModelList.length}})</span>
                    </span>
          <div class="disflex align-cen">
            <span @click="showDelTip(v.productsTypeId , k)" >
              <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/del.png"
                                alt="" class="w20 h20 pr16"
                                />
            </span>
            <span @click="to_up(v.productsTypeId , k)">
              <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/toup.png"
  ...
  export default {
    ...
    methods: {
      getInits(){
        ...
        WXAJAX.POST({ignore: true}, '', '/products/selectProductsTypeList').then((data) => {
      ...
      showDelTip(productsTypeId , index) {
        this.delTmp = { productsTypeId, index };
        this.delDialog = true;
      },
      del_tap(method) {
        if (method == 'cancel') {
      ...
      btn_tap(method,text){
        if(method == 'cancel'){
          ...
          if(this.edit_type == 'edit'){
            params.productsTypeId = this.editId ;
          }
          params.productsTypeName = text ;
          WXAJAX.POST(params, '', '/products/insertProductsType').then((data) => {
      ...
      to_up(productsTypeId ,index){
        ...
        }, '', '/products/moveProductsType').then((data) => {
      ...
      manage(method,productsTypeName,editId){
        if(method == 'edit'){//编辑
          this.edit_type = 'edit' ;
          this.dialog_ph = '分类名称' ;
          this.dialog_title = '修改分类';
          this.classify_name = productsTypeName ;
      ...
      del({productsTypeId , index}){
        wx.showLoading() ;
        WXAJAX.POST({
          productsTypeId : productsTypeId ,
        }, '', '/products/delProductsType').then((data) => {
