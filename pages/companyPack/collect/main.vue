<template>
	<div class="pt46">
		<!--top fix_top-->
		<div class="disflex bgfff textc jsaround fs16 lh44 c78 bbf7  zindex999 bgfff">
			...
			<span class="" :class="menu_id == 3 ? 'bbblue_2 cblue fbold' : '' " @click="menu_tap(3)">收藏机构</span>
		</div>
		<!--收藏企业-->

		<div v-if="menu_id == 3">
			<div class="bgfff pt15 pl15 pr15 pb15 disflex jsbet" v-for="(v,k) in prods" :key="k">
				...
				<div class="flex1 disflex jsbet align-cen">
					...
					<img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/del.png" alt=""
						class="w20 h20" @click.stop="del_tap(v.itemId,k)">
				</div>
			</div>
		</div>
		...
		<div v-else-if="menu_id == 2">
			<div class="bgfff pt10 pl9 pr16 pb15 disflex jsbet" v-for="(v,k) in prods" :key="k"
				@click="toDetail(v.itemId)">
				<img :src="v.photos" mode="aspectFill" alt="" class="w60 h60 bradius5 mr23">
				<div class="flex1">
					<p class="fs14 c38 over_2 pt3 "
          style="width: 400rpx;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{v.name}}</p>
			...
			<div class="bgfff pt10 pl9 pr16 pb15 disflex jsbet" v-for="(v,k) in prods" :key="k">
				...
				<div class="flex1">
					...
					<div class="disflex jsbet pt4 row-reverse align-cen">
						<img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/del.png" alt=""
							class="w20 h20" @click.stop="del_tap(v.itemId,k)">
						<span class="corange fs12" v-if="menu_id == 1">参考价 ￥{{v.price/100}}</span>
					</div>
				</div>
			</div>
		</div>
		...
		methods: {
			...
			inits() {
				...
				WXAJAX.POST({
					itemType: v.menu_id,
					// pageNum: v.page,
				}, '', '/personal/getCollection', '', '', 2).then((data) => {
					console.log(data, "获取收藏列表");
					if (data) {
			...
			del_tap(id, index) { //取消收藏
				...
				WXAJAX.POST(_request, '', _url, '', '', 2).then((data) => {
			...
			toDetail(el) { //查看文章详情
				uni.navigateTo({
					url: '/pages/companyPack/find/dynamicDetail/main?dynamicId=' + el + "&title=" + this.title
				})
			},
