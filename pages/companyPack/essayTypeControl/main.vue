  ...
  export default {
    ...
    methods: {
      ...
      btn_tap(method,text){
        if(method == 'cancel'){
          ...
          if(this.edit_type == 'edit'){
            params.essayTypeId = this.editId ;
          }
          params.name = text ;
          params.companyId = wx.getStorageSync('COMPANYID') ;
          params.enable  = 0 ;
          WXAJAX.POST(params, '', this.edit_type == 'edit'?'/essayType/update':'/essayType/add','','',1).then((data) => {
            wx.hideLoading();
      ...
      del({goodTypeId , index}){
        wx.showLoading() ;
        WXAJAX.POST({
          essayTypeId : goodTypeId ,
        }, '', '/essayType/deleteById','','',1).then((data) => {
