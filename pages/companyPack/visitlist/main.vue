<template>
  <div class="card-main w100p bgf5f6 bgfff"
     ...
     <div class="visit-rank bgfff">
         ...
         <span class="be8 lh25 pl9 pr10 bradius3 c80" @click="filtrate" v-if="screen==true"  style="font-size: 30rpx;position: absolute;right: 10rpx;z-index: 999999;">
               {{staffName}}
               <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/down.png" alt=""
                                 class="w10 h10"/>
         </span>
    ...
    <scroll-view :style="{height: scrollContentHeight+'px'}"
                 ...
                 @scrolltolower="scrolltolower" style="padding-bottom: 90rpx;">
      <!--搜索-->
      <div class="bgfff pl15 pb10 pr15"
           v-if="isLogin" style="display: none;">
        <div class="bgf5f6 bradius17 textc lh34 disflex align-cen pl25 search trans2 pr25"
          ...
          <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/search.png"
               ...
               alt=""
               class="w16 h16">
          <span class="searchText over_1"
                v-if="!isfocus">{{searchkey || '搜索姓名/电话号码'}}</span>
          <input type="text"
                 v-model="searchkey"
...
export default {
  ...
  onLoad(option) {
    if(option.types == 2){
      var remind = uni.getStorageSync('cardWelcomeSpeech')
      uni.showModal({
          title: '提示',
          content:'当前选择的机构是：'+ remind.company,
          showCancel:false,
      ...
      uni.showModal({
          title: '提示',
          content:'当前工作的机构是:'+remind.company+',是否需要切换名片,点击确定可以开始工作啦',
          success: function (res) {
              if (res.confirm) {

              } else if (res.cancel) {
                  uni.navigateTo({
                    url: "/pages/companyPack/mycard/main?type=1"
                  });
              }
          }
      });
  ...
  methods: {
    ...
    getAllCard() {
      ...
      let url='/visiting/listVisitingByInfo'
      let params={
        intervieweeName: '',
        intervieweePhone:'',
        intervieweeCardId:'',
      ...
      if(this.screen){
        url='/visiting/queryListCompanyByInfo'
        ...
        if (data && data.list) {
          ...
          data.list.forEach(function (i, k) {
            const cardData = {
              ...
              intervieweePosition: i.intervieweePosition || ''
            };
            if (i.state === 3) {//默认名片
              v.card_msg = cardData;
            } else {
    ...
    let choose_tabs_id=this.choose_tabs_id?this.choose_tabs_id:''
        console.log('ooo')
      console.log({
        // srcPath:'http://test1.mingyicloud.com/card/myshare.html?visitingId=&token='+wx.getStorageSync('token')+'&startTime='+this.startTime+'&endTime='+this.endTime+'&companyid'+this.companyid,
