  ...
  <div class="seckill">
    <div class="disflex jsbet bgfff pl15 pr15 pt15 pb15">
      <p class="over_2 fs14 c38 fbold flex1">
        ...
        {{proData["goodSpecModelList"][typeIdx]["goodSpecModelList"] && proData["goodSpecModelList"][typeIdx]["goodSpecModelList"][specIdx]
        && proData["goodSpecModelList"][typeIdx]["goodSpecModelList"][specIdx].specAttribute || ''}}
      </p>
      <button
  ...
  watch: {
    showSpecChoose(newVal) {
      if (!newVal) {
        ...
        this.currentSelectPro = this.proData.goodSpecModelList[0].goodSpecModelList[0]
      }
    }
  },
  data() {
  ...
  methods: {
    ...
    changeChoose(data, type, typeIdx, specIdx) {
      if (type === "spec") {
        ...
        this.currentSelectPro = data.goodSpecModelList[specIdx]
      }
      console.log("点击选择类型====》", data, type, typeIdx, specIdx)
      if (data && data.specId) {
         var killPrice = type =="typeIdx"? data.goodSpecModelList[specIdx].killPrice: data.killPrice;
          this.$emit('changeTypeId', data.specId,'isKill', killPrice);
      }
    ...
    //选择类型，规格弹框的按钮触发
    clickButton(payload) {
      console.log("当前的选择1111111=>", payload)
    ...
    car(payload) {
      ...
      this.$emit("car", {
        num: payload.sum,
        specIds: this.goodsData.specsId
      });
      // setTimeout(() => {
    ...
    submitOrder(payload) {
      ...
      // this.currentSelectPro.price = (this.currentSelectPro.killPrice / 100).toFixed(
      //   2
      // );
      ...
      this.currentSelectPro.price = this.goodsData.killPrice;
      this.currentSelectPro.allPrice =  this.goodsData.killPrice * this.currentSelectPro.num;
      this.currentSelectPro.specIds = this.goodsData.specsId
      let tmpObj = {
        companyId: this.proData.companyId || "",
        companyName: this.proData.companyName || "",
        orderState: 1,
        allNum: 1,
