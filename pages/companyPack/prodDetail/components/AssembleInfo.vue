  ...
  <div class="assemble-info">
    ...
    <div class="disflex jsbet bgfff pl15 pr15 pt15 pb15">
      <p class="over_2 fs14 c38 fbold flex1">
        {{proData.goodsName || ''}}
        {{proData.goodSpecModelList[typeIdx].specName || ''}}
        {{proData.goodSpecModelList[typeIdx]["goodSpecModelList"] && proData.goodSpecModelList[typeIdx]["goodSpecModelList"][specIdx].specAttribute || ''}}
      </p>
      <button
      ...
      >
        <img
          src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/share2.png"
          alt
          class="w15 h15 mr7"
        />
    ...
    <div class="pl15 pr15 type-specs flex-sb-c bgfff" @click="showSpecChooseModel">
      ...
      <div class="" @click="clickRightRowEvent">
        <img
          src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/right_row.png"
          alt
    ...
    <!-- 更多拼团 -->
    <div class="more-collage mt11" v-if="proData.assembleModelList && proData.assembleModelList.length>0">
      ...
      <AssembleOrderItem @join="joinGroup" v-if="proData.assembleModelList[1]" :orderInfo="proData.assembleModelList[1]"></AssembleOrderItem>
    ...
    ></GoodsSpecChoose>
    <AssembleOrderListBox @join="joinGroup" :lists="proData.assembleModelList" :isShow="isShowOrderList" @close="closeOrderList"></AssembleOrderListBox>
  </div>
</template>
<script>
...
export default {
  ...
  showSpecChoose(newVal) {
      if (!newVal) {
        ...
        this.currentSelectPro = this.proData.goodSpecModelList[this.typeIdx].goodSpecModelList[this.specIdx]
      }
    }
  },
  mounted(){
  ...
  methods: {
    ...
    group(payload) {
      ...
      wx.showLoading();
      WXAJAX.POST({ goodsId: payload.goodsId, assembleId: payload.assembleId||'' }, '', '/orders/openOrJoinAssemble').then(data => {
        wx.hideLoading();
        if (data) {
    ...
    //提交订单，
    submitOrder(payload) {
      //console.log(this.proData.goodSpecModelList)
      // if(this.goodsData.specsId.length < this.proData.goodSpecModelList.length){
      //   wx.showToast({
      //     title:'请选择你要的商品信息',
      //     icon:'none'
      //   })
      ...
      this.currentSelectPro.specName = this.goodsData.typeNames;
      let price = this.buyType==='alone'? this.goodsData.goodsPrice : this.goodsData.assemblePrice.toFixed(2);
      this.currentSelectPro.price = this.buyType==='alone'? this.goodsData.goodsPrice : this.goodsData.assemblePrice;
      this.currentSelectPro.allPrice = this.buyType==='alone'? this.goodsData.goodsPrice * this.currentSelectPro.num : this.goodsData.assemblePrice * this.currentSelectPro.num;
      this.currentSelectPro.specIds = this.goodsData.specsId
      let tmpObj = {
        ...
        orderState: 1,
        allNum: 1,
        orderPrice:this.buyType==='alone'? this.goodsData.goodsPrice * this.currentSelectPro.num : this.goodsData.assemblePrice * this.currentSelectPro.num,
    ...
    //选择类型和规格的浮窗
    changeChoose(data, type, typeIdx, specIdx) {
      console.log(this.proData)
      if (type === 'spec') {
      ...
      if (data && data.specId) {
        this.$emit('changeTypeId', data.specId,'isAssemble',data.assemblePrice);
        this.$set(this.proData,"assemblePrice",'isAssemble',data.assemblePrice)
      }
      this.typeIdx = typeIdx
      this.specIdx = specIdx
      // console.log("执行了拼团",this.proData.assemblePrice)
    },
