  ...
  <div class="h100p pb44">
    ...
    <div class="w100p h375">
      ...
      <div v-else class="w100p h100p bgfff borderbox  textc pt123" @click="uploadpic(-1)">
        ...
        <p class="fs14 ca8">图片大小不得超过4mb，图片格式最佳为800*800</p>
      </div>
    ...
    <div class="bgfff fs14 ca8 mb10" v-for="(v,k) in prodSize" :key="v">
      <div class="lh44 disflex jsbet align-cen  pl16 pr15">
        ...
        <div class="disflex align-cen">
          <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/product-index/20190425173426.png"
               @click="delSize(k)"
               alt="" class="w20 h20"
          />
        </div>
    ...
    <div class="mb10 bgfff fs14 ca8">
      ...
      <div class="disflex align-cen lh44 posre pl42" v-if="modelSwtich">
        <label class="checkBox" :class=" isPublic ? 'active' : '' "
               @click="isPublic = !isPublic">
          <span></span>
        </label>
        <span class="c38 pl11">申请发布到产品中心</span>
      </div>
    </div>
    ...
    <div class="bgfff textc fs14 ca8 pb36 mb44">
      ...
      <span class="disinblock cblue bblue lh30 bradius5 pl11 pr11 mb10"
            @click="page_turn('productIntro' , '?type=prod')">
  ...
  export default {
    ...
    methods: {
      ...
      bottom_tap(method) {
        ...
        let prodSize = JSON.parse(JSON.stringify(this.prodSize)), v = this;
        console.log(prodSize)
        for(var i = 0 ; i < prodSize.length; i++ ){
          for(var j=0; j< prodSize[i].list.length; j++) {
            if(prodSize[i].list[j].price == '' || prodSize[i].list[j].price <= 0){
              wx.showToast({
                title: '金额不能小于0！',
                icon: 'none'
              });
