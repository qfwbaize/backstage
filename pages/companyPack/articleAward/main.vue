      ...
      <scroll-view :style="{height: scrollContentHeight+'px'}"
          ...
          :key="info.id">
            ...
            <!-- <div class="flex-base bottom h44 pl15 pr15">
              <div class="flex-base">
                 <span class="fs12 ca8">时间:{{ info.createTimeStr }}</span>
              </div>
            </div> -->
            <div class="flex-base bottom h44 pl15 pr15">
              <div class="flex-base">
                 <span class="fs12 ca8 pl15">{{ info.rewardExplain }}</span>
              </div>
            </div>
          </div>
        </scroll-view>
...
export default {
  ...
  methods: {
    ...
    getlist(page){
      let url = "/userAccount/listDynamicAccountDetails"
      let params=[];
      if(this.status==2){
        params={
          pageSize:10,
    ...
    exportlist(){
      ...
      if(wx.getStorageSync('isBoss')){
        url='/userAccount/exportCompanyAccountDetailsListExport'
      }else{
        url='/userAccount/exportDynamicAccountDetailsListExport'
      }
      let params={};
      if(this.status==2){
        params={
