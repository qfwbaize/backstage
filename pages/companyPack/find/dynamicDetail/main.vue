<template>
  <div class="pb50 posre h100p">
    <!-- <view @click="lingqu"  style="white-space:pre-wrap;text-align:center;width:160rpx;height:60rpx;background:#51cdcc;color:#fff;">
       领取奖励测试修改
    </view> -->
    <div class="headframe">
      <image style="width: 112rpx;height: 112rpx;border-radius: 50%;" :src="image" mode=""></image>
      <div style="display: flex; flex-direction: column;margin-left: 30rpx;">
        <span style="font-size: 28rpx;margin-top: 10rpx;margin-bottom: 16rpx;font-weight: bold;">{{name}}</span>
        <span style="font-size: 26rpx;color: #8F8F8F ;">{{companyName}}</span>
      </div>
      <div class="bblue disborder align-cen w80 h25 disflex jscen fs14 cblue bradius15" style="position: absolute; top: 42rpx;right: 60rpx;"
        @click="talk">
        联系TA
      </div>
    </div>
    <p class="pt15 fbold fs18 c38 pl15 pr27 bgfff " style="word-break: break-all;">{{title}}</p>
    <div class="px15 dec">
      <div class="left-dec">
        <span>
          {{headDynamicAuthor}}
    ...
    <div class="bottom pb50 borderbox bgfff flex1">
      <div class="disflex bbf5f6 c78">
        <div class="w50p h44 disflex posre jscen align-cen fs14" :class="currentTab == 3 ? 'active': ''" @click="tab_tap(3)"
          v-if="topic==true">
          <span class="mr10 fs14">回答问题</span>
          <!-- {{dynamic.thumbsModelsList.length || 0}} -->
          <span class="line"></span>
        </div>
        <div class="w50p h44 disflex posre jscen align-cen fs14" :class="currentTab == 1 ? 'active': ''" @click="tab_tap(1)">
          <span class="mr10 fs14">评论</span>
          {{dynamic.commentModelList.length || 0}}
          <span class="line"></span>
        </div>
        <div class="w50p h44 disflex posre jscen align-cen fs14" :class="currentTab == 2 ? 'active': ''" @click="tab_tap(2)">
          <span class="mr10 fs14">赞</span>
          {{dynamic.likeNum || 0}}
          <span class="line"></span>
        </div>
        <button class='bgfff fs12 cdeepblue p0 w33p disflex jscen align-cen' @click="toShare()" hover-class="other-button-hover">
      ...
      <div class="content">
        <div v-if="currentTab == 1">
          ...
          <div class="disflex flex-column">
            ...
            <div class="comment-list bbf5f6 pt20 pb20 bgfff disflex" v-for="(v,k) in dynamic.commentModelList" :key="k">
        ...
        <div v-else-if="currentTab == 2">
          <div class="disflex pl15 pt10 bbf5f6  pb10 borderbox bgfff" v-for="(item,index) in dynamic.thumbsModelsList"
            :key="index">
      ...
      <!-- <view  class="pt10 pb10 pl10 pr15 disflex jsbet align-cen wenzhang"
	   style="margin:20rpx;background-color: #fff;">
	  		<view class="w100p">
	  			<view class="disflex jsbet w100p">
	  				<text>添加机构视频(0/1)</text>
					<view  class="disflex jsbet" >
						<text class="wzadd">+</text>
						<text style="color: rgb(1, 196, 161);">添加</text>
					</view>
	  			</view>
				<view class="pt20 pb20 fs14" style="color: #8D8D8D;">
					<text>还未添加产品,请添加</text>
				</view>
	  		</view>
	  </view>
	  <view  class="pt10 pb10 pl10 pr15 disflex jsbet align-cen wenzhang"
	  ...
	  <view  class="pt10 pb10 pl10 pr15 disflex jsbet align-cen wenzhang"
	   style="margin:20rpx;background-color: #fff;margin-top: 20rpx;">
	  		<view class="w100p">
	  			<view class="disflex jsbet w100p">
	  				<text>添加机构论文(0/3)</text>
	  					<view  class="disflex jsbet" >
	  						<text class="wzadd">+</text>
	  						<text style="color: rgb(1, 196, 161);">添加</text>
	  					</view>
    ...
    <!-- <div class="btns disflex bgfff">
      <div id="navbottom" class="disflex w155 jscen">
    ...
    <!-- <div class='bgfff ca7 fs12 lh1 cdeepblue p0 w33p disflex flex-column jscen align-cen' @click.stop="addComment('','','add')">
          <img class="w20 h20 mb2" :src="commentIcon">
          <span class="ca7 lh1">评论</span>
        </div>
      </div> -->
    <!-- <div class="textc jscen fs18 cfff align-cen flex1 consulting" @click="talk"> -->
    <!-- <img :src="ommentIcon" class="w24 h24 mr7"> -->
    <!-- 200￥引用此文章转发
		<p class="fs14 ce8">本文作者设置了付费引用功能</p>
      </div> -->
    <!--评论输入框-->
    <div class="fix_bottom disflex borderbox align-cen pl15 h49 bgfff edit_input trans2" v-if="input_show==true"
      @click.stop="">
      <!--:class="input_show ? '' : 'hide' "-->
      <div class="bgfff  disflex w100p jsbet pr11 borderbox align-cen">
        <input fixed="true" class=" borderbox h40 lh40 pr11 bgf5f6 bradius5 flex1 mr11 comment_input c38 fs18 pl15"
          :focus="comment_focus" :placeholder="
            replayComment.commentId
              ? '回复 ' + replayComment.userName + '：'
              : '评论'
          "
          cursor-spacing="4" @focus="inputgetFocus(this, $event)" @confirm="inputConfirm" :style="{ height: text_height + 'px' }"
          v-model="comments" />
        <!-- <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/smile.png" alt="" class="w25 h25" @click="smile_tap"> -->
        <span class="bgblue cfff w50 textc lh30 fs16 bradius5 disblock ml10" @click.stop="inputConfirm">发送</span>
      </div>
    </div>
    <!-- </div>
    ...
    <!-- 分享弹窗 -->
    <cover-view class="w100p h100p prod_choose_class borderbox disflex column-reverse" :class="{show : isShowShare} "
      ...
      <cover-view class="bgfff inner textc lh44">
        <cover-view class="share-box">
          ...
          <cover-view style="margin-left: 136.5upx;" @click="onShareEvent">
            <cover-image class="share-img mb8" src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/weixin_share2.png"></cover-image>
            <cover-view class="share-title">名片海报</cover-view>
          </cover-view>
          <cover-view style="margin-left: 136.5upx;" @click="onShareInSide">
            <cover-image class="share-img mb8" src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/weixin_share.png"></cover-image>
    ...
    <div class="mask disflex align-cen jscen" v-if="isPreviewImg">
      <div class="disinblock ">
        ...
        <span @click="saveImg" class="bg_line_blue cfff textc fs18 fbold bradius5 lh44 mb15" style="width: 465rpx;display: block;margin: 0 auto">保存图片</span>
      </div>
    </div>
    <SHARE v-if="share==true" :dynamicId="dynamicId" :companyId="companyId" saveType='1' type='1' @closeShare="closeShare"></SHARE>
    ...
    <div class="positionFixed">
      <button class=' fs12 w50 h50 disflex jscen align-cen bradius50p' style="padding: 0;background-color:#51cdcb; ;"
        @click="toShare()" hover-class="other-button-hover">
        <img class="w20 h20" :src="shareIcon">
      </button>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    async onShareAppMessage(e) {
      // var currentCompany=this.currentCompany;
      // if(typeof this.currentCompany=='string'){
      //   currentCompany=JSON.parse(this.currentCompany)
      // }

      // const { companyId, cardId } = currentCompany;
      ...
      let path = '/pages/companyPack/find/dynamicDetail/main?dynamicId=' + this.dynamicId + '&companyId=' + wx.getStorageSync(
    ...
    async onLoad(options) {
      ...
      // this.cardId = cardId;
      await this.getInits(options.cardId)
      //判断是否已经收藏
        ...
        try {
          await WXAJAX.POST({
            ...
            if (data) {
              ...
              v.headDynamicAuthor = data.headDynamicAuthor
              v.name = data.name
              ...
              let timer1 = setTimeout(function() {
                ...
                //   // 节点的上边界距离顶部距离
                //   that.navbottomheight = rect.height
...
<style scoped>
  ...
  .wzadd {
    border: 2px rgb(1, 196, 161) solid;
    color: rgb(1, 196, 161);
    border-radius: 50%;
    font-weight: bold;
    text-align: center;
  ...
  .headframe {
    width: 100%;
    height: 134rpx;
    /* box-shadow: 0rpx 2rpx 8rpx 0rpx rgba(196, 241, 238, 100); */
