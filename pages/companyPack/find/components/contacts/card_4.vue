<template>
    <div class="bgfff pl15 pt15 pr15 card4" @click="makePhone(card_msg.companyName)">
        <div class="posre w100p h100p card4-box">
            <img :src="card_msg.logo || card2_logo"
                  mode="aspectFill"
                  class="card4_bg">
            <p class="fs20 mt15 mb14 c38 fbold">{{card_msg.name || '姓名'}}</p>
            <p class="fs14 mb10 c78">{{card_msg.position || '职位'}}</p>
            <span class="card4-company fs12 cblue">{{card_msg.companyName || '公司名称'}}</span>
            <!-- <div class="cacrd3_left">
                <p class="fs12 cfff pb6" @click="makePhone(card_msg.tel)">{{card_msg.tel || '电话'}}</p>
                <p class="fs12 cfff">{{card_msg.email || '邮箱'}}</p>
            </div>
