<template>
  <div>
    <div class="top_card fs12 mb15 shadow_gray pb28" v-if="type == 'plus'">
      <div class="pl34 pt25 cblue fs12"><span class="before_line_3">{{card_msg.companyName || '机构名称'}}</span></div>
      <div class="top_msg disflex pt25 jsbet"  @click="page_turn('index')">
        <div>
          <p class="fbold c38 fs18">{{card_msg.name || '姓名'}}</p>
          <p class="fs140 ca8 pt10 pb23">{{card_msg.position || '职位'}}</p>
          <p class="fs12 c78">» {{card_msg.phone || '电话'}}</p>
          <p class="fs12 c78">» {{card_msg.email || '邮箱'}}</p>
        </div>
        <img :src="card_msg.logo || card1_logo"  mode="aspectFill" alt="" class="w60 h60 bradius5" />

      </div>
      <div class="disflex jsbet pl25 pr27 pt33">
        <div class="disflex">
          <span class="cblue fs12 mr20" v-if="!hasCard">NO.{{index }}</span>
          <span @click.stop="makePhone"
                v-if="!hasCard" >
            <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/tel.png"
                              alt="" class="w16 h16 mr20"
                              />
          </span>
          <!--   <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/user/share.png"
                  alt="" class="w16 h16"
                  @click.stop="share(card_msg.companyId,card_msg.cardId)">-->

          <button class='w16 h16 bgfff contentbox textr disflex align-cen textr pl0 pr0'
                  id="111"
    ...
    <div class="pt15 pl16 pr15 pt16 bgfff pb15" v-else @click="page_turn('index')">
      <div class="top_card fs12 shadow_gray pb42">
        ...
        <div class="top_msg disflex jsbet pt40">
          <div>
            <p class="fbold c38 fs18">{{card_msg.name || '姓名'}}</p>
            <p class="fs14 ca8 pt10 pb25">{{card_msg.position || '职位'}}</p>
            <p class="fs12 c78 pb10">» {{card_msg.phone || '电话'}}</p>
            <p class="fs12 c78">» {{card_msg.email || '邮箱'}}</p>
          </div>

          <div class="disflex">
            <img :src="card_msg.logo || card1_logo" alt="" mode="aspectFill" class="w100 h100 bradius5" />
          </div>
        </div>
      </div>
    </div>
