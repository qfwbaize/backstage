<template>
	<view>
      ...
      <view class="mainlist" @click="toTaskDetail(item.taskCustomerId)" v-for="(item,index) in tasklist" :key="index">
        <view class="taskinfo">
		...
		<div v-if="tabType==2" class="task" style="padding-bottom: 84rpx;">
			<view class="mainlists" @click="toBiddingTask(item.taskCustomerId)" v-for="(item,index) in biddingList" :key="index">
			  <view class="taskinfo">
			    <view class="taskinfo_title">{{item.taskName}}</view>
...
<script>
	...
	export default {
      ...
      task_button() {
        uni.navigateTo({
          url:'../../demand/newTask/newTask?taskType=0'
        })
      },
      //跳转到我竞标的页面
