<template>
  <view class="content">
    ...
    <view class="mainlist">
      ...
      <view class="task_button" v-if="missionDetail.status == 0 && missionDetail.type != 4">
      ...
      <view class="task_btn" v-if="missionDetail.status == 1"  @click="sonMission">创建子任务</view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toNewTask(){
        uni.navigateTo({
          url:'../newChildMission/newChildMission?pid='+this.id+'&title='+this.missionDetail.title+'&status='+this.missionDetail.status+"&pattern=1"+"&dynamicid="+this.missionDetail.pro_id
        })
      },
      //点击任意关闭
      off() {
      ...
      refuse(){
        ...
        http({url:'/apis/companytask/del',method:'DELETE',data:data}).then(res=>{
      ...
      getMissionDetail(id){
        ...
        http({url:'/apis/companytask/read',method:'GET',data:data}).then(res=>{
          if(res.data.code == 200){
            ...
            // console.log(this.missionDetail,777777777)
          }
        })
      },
      //获取文章详情页
