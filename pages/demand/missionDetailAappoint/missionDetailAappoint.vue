    ...
    <view class="mainlist">
      ...
      <view class="contract" v-if="type != 1">
        <view class="contract_top" v-for="(peopleItem,peopleIndex) in peopleList" :key="peopleIndex">
          ...
          <view class="contract_statusDetail" style="margin-right: 0;" @click="toProgressDetail(peopleItem.card_id,peopleItem.id,peopleItem.task_id,peopleItem.status)">查看详情</view>
        </view>
      </view>
     ...
     <view class="contract" v-for="(item,index) in evidenceInfo" :key="index">
      ...
      <view class="detailtext" v-if="payment != ''">
        <view class="squares" style=" margin-right: 40rpx; background-color: rgba(26, 208, 156, 100);"></view>
        <view style="width: 280rpx;">打款凭证</view>
      </view>

      <view class="contract" style="padding-top: 20rpx;" v-if="payment != ''">
        <view class="contractText">{{payment.create_time}}</view>
        <view class="contractText">任务已完成验收 奖励金额{{payment.money}}</view>
        <view class="contract_speed">
          <view class="" v-for="(s,k) in payment.imgUrl" :key="k">
            <image :src=s mode=""></image>
          </view>
        </view>
      </view>

      <view class="button" v-if="missionDetail.status == 2 && type != 1" @click="toImg">上传打款凭证</view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toImg(){
        uni.navigateTo({
          url:'../upLoadImg/upLoadImg?company_task_id='+this.missionDetail.company_task_id+'&task_id='+this.missionDetail.id+'&type=2'
        })
      },
      //来自其他机构的打款
      getPayment(company_id,id){
        var data = {
          company_id:company_id,
          task_id:id
        }
        http({url:'/apis/companytask/read_attor_company_reward',method:'GET',data:data}).then(res=>{
          if(res.data.code = 200){
            this.payment = res.data.data
            this.payment.imgUrl = []
            this.payment.imgUrl = this.payment.contract.split(',')
          }
        })
      },
      //向主任务申请打款
      ...
      //跳转到员工任务进度详情
      toProgressDetail(e,id,taskid,status){

        uni.navigateTo({
          url:'../progressDetail/progressDetail?cardId='+e+'&id='+id+'&taskId='+taskid+'&type='+this.missionDetail.type+'&status='+status
        })
      },
      //获取任务指定人的列表
      getMissionPeople(id){
      ...
      getMissionDetail(id){
        ...
        http({url:'/apis/companytask/read',method:'GET',data:data}).then(res=>{
          if(res.data.code = 200){
            ...
            this.getPayment(this.missionDetail.company_id,this.missionDetail.id)
