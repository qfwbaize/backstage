	...
	<view class="content">
		...
		<view class="lists">
			...
			<view class="" v-show="activeIndex == 0">
				<view class="list" v-for="(item,index) in mePromoteUserinfo" :key="index" @click="toDetail(item,index)">
					<view class="title" @click.stop="toArticleDetail(item)">
					...
					<view class="reward">设置奖励：{{item.pushEffectRewardAmount}}元</view>
				</view>
				<!-- 暂无信息 -->
				<view class="noMsg" v-if="mePromoteUserinfo.length == 0">
					<view class="noYet">
			...
			<view class="" v-show="activeIndex == 1">
				<view class="promote" v-for="(item,index) in promoteMeUserinfo" :key="index"
            ...
            <view class="">剩余时间：{{item.times}}</view>
						<!-- <view class="">当前转发人数：8人</view> -->
					</view>
					<!-- <view class="promote_text">
						<view class="">当前转发总数：40次</view>
						<view class="">剩余时间：{{item.times}}</view>
					</view> -->
					<button class="forward_read" @click.stop="toShare(item.dynamicId,item.companyId,item.title,item.photos,item)">立即转发</button>
				</view>
				<!-- 暂无信息 -->
				<view class="noMsg" v-if="promoteMeUserinfo.length == 0 ">
					<view class="noYet">
						<image src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/product-index/20190418164236.png"
							mode="">
						</image>
    ...
    components: {
		...
		data() {
			return {
				...
				pageSize: 10,
				activeIndex: 0,
				mePromoteUserinfo: [], //推效我发的
				promoteMeUserinfo: [], //推效发给我的
				open: false,
      ...
      // },
			...
			toArticleDetail(item) {
				if (item.type == 1 || item.type == 3) {
					let url = '../../companyPack/find/dynamicDetail/main?dynamicId=' + item.dynamicId + '&insideShareId=' +
						item
			...
			MePromote() {
				...
				WXAJAX.POST(data, '', '/pushEffect/record/list/pushEffectDynamic', '', '', 1).then(res => {
					if (res !== undefined) {
						if (this.pageNum == 1) {
							this.mePromoteUserinfo = res.list
						} else {
							this.mePromoteUserinfo = [...this.mePromoteUserinfo, ...res.list]
						}
						for (var i = 0; i < this.mePromoteUserinfo.length; i++) {
							this.mePromoteUserinfo[i]['time'] = this.mePromoteUserinfo[i].createTimeStr.split(' ')[
								0]
							if (this.mePromoteUserinfo[i].rewardExpireTimeStr == undefined) {
								this.mePromoteUserinfo[i]['times'] = "活动已到期"
							} else {
								this.mePromoteUserinfo[i]['times'] = this.mePromoteUserinfo[i].rewardExpireTimeStr
									.split(' ')[0]
							}
						}
			...
			promoteMe() {
				...
				WXAJAX.POST(data, '', '/pushEffect/record/list/pushEffectMyDynamic', '', '', 1).then(res => {
					if (res !== undefined) {
						if (this.pageNum == 1) {
							this.promoteMeUserinfo = res.list
						} else {
							this.promoteMeUserinfo = [...this.promoteMeUserinfo, ...res.list]
						}
						for (var i = 0; i < this.promoteMeUserinfo.length; i++) {
							this.promoteMeUserinfo[i]['time'] = this.promoteMeUserinfo[i].createTimeStr.split(' ')[
								0]
							if (this.promoteMeUserinfo[i].rewardExpireTimeStr == undefined) {
								this.promoteMeUserinfo[i]['times'] = "活动已到期"
							} else {
								this.promoteMeUserinfo[i]['times'] = this.promoteMeUserinfo[i].rewardExpireTimeStr
									.split(' ')[0]
							}
						}
						if (res.list.length < this.pageSize) {
