<template>
	<div class="bgfff disflex flex-column ">
		<div class="flex1 overauto">
			<div class="disflex align-cen">
				<picker class="w100p" :range="setDemandClass" :value="index" @change="bindDateChange">
					<div class="disflex w100p borderbox jsbet align-cen pt10 pl18 pb10 pr15 h49">
						<div class="">
							选择分类
						</div>
						<div class="disflex align-cen">
							<p class="fs16 ca7 mr10">{{ setDemandClass[specificDate] || '请选择'}}</p>
			...
			<div class="pl15 pr15 pt  15 pb15 posre borderbox">
				...
				<textarea class="w100p pl15 pr15 pb15 pt15 pb28 bgf5f6 borderbox bradius5" maxlength="200"
					@blur="text_tap" placeholder="请输入内容"></textarea>
			...
			<div class="pl15 pr15 pt  15 pb15 posre borderbox">
				...
				<textarea class="w100p pl15 pr15 pb15 pt15 pb28 bgf5f6 borderbox h90 bradius5" maxlength="50"
					@blur="remarks_tap" placeholder="请输入内容"></textarea>
			...
			<div class="pl15 disflex h49 align-cen" @click="singleCurrent_tap">
				<div class="w20 h20 bradius10 b2e8 disflex jscen align-cen mr8 " style="transition: all .4s"
	...
	export default {
		...
		methods: {
			type() {
				...
				let that = this
				WXAJAX.POST(data, '', '/demandSquare/queryAllClassification', '', '', 2).then(data => {
					if (data) {
						that.list = data;
						wx.hideLoading()
						for (var i = 0; i < data.length; i++) {
							that.setDemandClass.push(data[i].name)
						}
					} else {
						that.setDemandClass = [];
					}
			...
			bindDateChange(e) {
				...
				this.settingId = this.setDemandClass[this.specificDate]
			},
			text_tap(e) {
			...
			agreemen_tap() {
				wx.navigateTo({
					url: '/pages/companyPack/agreement/main'
				})
			},
			confirm_tap() {
				...
				for (var i = 0; i < this.list.length; i++) {
					if (this.settingId == this.list[i].name) {
						id = this.list[i].settingId
					}
				}
				let data = {
				...
				WXAJAX.POST(data, '', '/demandSquare/addDemandSquare', '', '', 2).then(data => {
						uni.showToast({
							title: '已提交!'
						})
						setTimeout(() => {
