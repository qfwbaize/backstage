<template>
  <div class="h100p toppage" :class="pageFixed ? 'fix' : '' " id="CONT">
    <div class=" w100p  bgfff h30 lh30 ">
      <div style="position: fixed;">群名称</div>
      <div @click="institutions" style="position: fixed; right: 20rpx;">+++</div>
      <div class="b1s cfff fs14 textc " v-if="institution == true"
        style="width: 200rpx;position: fixed;right: 20rpx;top: 60rpx;background-color: #4c4c4c;">
        <div class="h30 lh30" style="border-bottom: 1px solid #5b5b5b;" @click="toupdate">添加成员</div>
        <div class="h30 lh30" style="border-bottom: 1px solid #5b5b5b;">修改成员</div>
        <div class="h30 lh30" style="border-bottom: 1px solid #5b5b5b;">群员列表</div>
      </div>
    </div>
    <!--IM内容-->
    <div class="chat-box" @click="chatBoxClickEvent" @touchmove="chatBoxClickEvent">
      <div class="im_by_time" v-if="msgGroupCont" v-for="(msgitem,k) in msgGroupCont.data" :key="k">
        <!--时间-->
        ...
        <div class="textc" v-if="msgitem.messageType == 4">
          <div :class="{'sys-message': msgitem.sendId == myid}">
            ...
            <div v-else class="sys-messages">
              <view>{{seedName}} 已向你发送了联系方式</view>
              <view class="disflex">尽快与他联系<text @click="callPhone(msgitem.message)"
                  style="font-weight: bold;color: #51cbcd;margin-left: 10upx">{{msgitem.message}}</text>
              </view>
            </div>
          </div>
        </div>
        <template v-else>
          ...
          <div class="im_right disflex pl22 pr15 im_item flex-end" v-if="msgitem.sendId == myid">
            ...
            <div v-if="msgitem.contentType && msgitem.contentType === 3"
              style="margin-top: 30rpx; margin-right: -40rpx;">
              <div class="disinblock">
                <div class='output-audio-right'>
                  <div class='disflex align-cen flex-end audio-right' @click="audioPlay(msgitem.message, k)">
                    <text style="margin-right: 10upx;color: #94a5a5"
                      class="fs12">{{msgitem.duration ? `${msgitem.duration}"` : ''}}</text>
                    <img class='ico-right' v-if="urls !== msgitem.message" src='/static/images/play-voice.png' />
                    <img class='ico-right' v-else src='/static/images/play-voiceimg.gif' />
                  </div>
                </div>
              </div>
            </div>
            <!--图片-->
            <div v-else-if="msgitem.contentType === 2" style="margin-top: 30rpx; margin-right: -40rpx;">
              <img mode="widthFix" :src="msgitem.message" @click="previewImg(msgitem.message)"
                style="border-radius: 10rpx;max-width: 130rpx" />
            </div>
            <!--视频-->
            <div v-else-if="msgitem.contentType === 4" style="margin-top: 30rpx; margin-right: -40rpx;">
              <video style="border-radius: 10rpx;max-height: 310rpx;max-width: 310rpx" controls="false"
                :src="msgitem.message"></video>
            </div>
            <!--信息-->
            <div v-else style="margin-top: 30rpx; margin-right: -40rpx;">
              ...
              <div class="im_left_msg im_msg pl8 pr23 pb15 pt14 bgfff bradius5 w250 be8 posre disflex"
                @click="toGoodsDetail(msgitem.message)" v-if="msgitem.messageType==3">
              ...
              <div class="im_right_msg  pl15 pt14 pr18 bgfff bradius5 w250 be8 posre" v-if="msgitem.messageType==2">
                <p class="fs14 c38">
                  你好，我是{{msgitem.message.companyName}}的{{msgitem.message.name}}，欢迎进入我的名片，有什么可以帮到你的吗？
                </p>
                <p class="fs12 ca8 pt10 pb9">您还可以点击查看</p>
                <div class="disflex jsbet fs12 c38 lh30 textc wrap">
                  <span class="be8 bgf5f6 borderbox w104 mb10 " @click="page_turn('Products')">机构产品</span>
                  <span class="be8 bgf5f6 borderbox w104 mb10" @click="page_turn('index')">我的名片</span>
                  <span class="be8 bgf5f6 borderbox w104 mb10 " @click="page_turn('WebSite')">机构官网</span>
                  <span class="be8 bgf5f6 borderbox w104 mb10" @click="page_turn('Dynamic')">机构动态</span>
                </div>
                <span class="triangle_border_right"></span>
              </div>
            </div>
            <!--头像-->
            <span class="fs10">{{msgitem.businessCard.name}}</span>
            <img :src="msgitem.businessCard.logo" alt="" mode="aspectFill" class="im_right_img ml10 w40 h40 bradius5">
          </div>

          <!--left-->
          <div class="im_left disflex pl15 pr22 im_item" v-if="msgitem.sendId !=myid">

            <!--头像-->
            <img :src="msgitem.businessCard.logo" mode="aspectFill" alt="" class="im_left_img mr10 w40 h40 bradius5">
            <span class="fs10">{{msgitem.businessCard.name}}</span>

            <!--语音-->
            <div v-if="msgitem.contentType && msgitem.contentType === 3"style="margin-top: 30rpx; margin-left: -40rpx;">
              <div class="disinblock">
                <div class='output-audio-left'>
                  <div class='disflex align-cen flex-end audio-left' @click="audioPlay(msgitem.message, k)">
                    <text style="margin-right: 10upx;color: #94a5a5"
                      class="fs12">{{msgitem.duration ? `${msgitem.duration}"` : ''}}</text>
                    <img class='ico-left' v-if="urls !== msgitem.message" src='/static/images/play-voice.png' />
                    <img class='ico-left' v-else src='/static/images/play-voiceimg.gif' />
                  </div>
                </div>
              </div>
            </div>
            <!--图片-->
            <div v-else-if="msgitem.contentType && msgitem.contentType === 2" style="margin-top: 30rpx; margin-left: -40rpx;">
              <img mode="widthFix" :src="msgitem.message" @click="previewImg(msgitem.message)"
                style="border-radius: 10rpx;max-width: 130rpx" />
            </div>
            <!--视频-->
            <div v-else-if="msgitem.contentType && msgitem.contentType === 4"style="margin-top: 30rpx; margin-left: -40rpx;">
              <video style="border-radius: 10rpx;max-height: 310rpx;max-width: 310rpx" controls="false"
                :src="msgitem.message"></video>
            </div>
            <!--信息-->
            <div v-else style="margin-top: 30rpx; margin-left: -40rpx;">
    ...
    <EditInputer :isInput="isInput" :isMore="isMore" :isPadding="isPadding" :phrases="phrases" :text="text"
      @clickPhrasesEvent="clickPhrasesEvent" @closeMore="isMore = false" @closePhrases="phrases = false"
      @voiceIconEvent="isInput = false" @moreIconEvent="moreIconEvent" @keyboardEvent="isInput = true"
      @touchStartEvent="touchStartEvent" @touchEndEvent="touchEndEvent" @clickPhotoEvent="clickPhotoEvent"
      @clickShotEvent="clickShotEvent" @clickVideoEvent="clickVideoEvent" @inputBlurEvent="isMore = phrases = false"
      v-on:comment="getcomment" :titleLists="titleLists" />
  ...
  export default {
    ...
    onReady() {
      ...
      let param = {
        ...
        clientNum: 0,
        pageSize: 20,
        cardId: this.chatGroupId,
        targetId: this.chatGroupId,
        targetCardId: this.chatGroupId,
      }
      this.params = param;
      WebSocket.getChatGroup(param);
      wx.setNavigationBarTitle({
        title: this.$root.$mp.query.groupName
      });
    ...
    async onPullDownRefresh() {
      this.params.clientNum = this.msgGroupCont.data.length;
    ...
    methods: {
      toupdate() {
        uni.navigateTo({
          url: './update?&chatGroupId=' + this.chatGroupId
        })
      },
      //编辑群
      institutions() {
        uni.navigateTo({
          url: "./EditGroup?&id=" + this.chatGroupId
        })
      },
      ...
      getCommonPhrases() {
        ...
        WXAJAX.POST({
            pageSize: 99999,
            pageNum: 1
          }, '', '/useful/own/queryList', '', '', 1).then((data) => {
            if (data.list) {
              this.list1 = data.list
              this.titleLists = this.list1.concat(this.list2, this.list3)
              this.titleLists = this.titleLists.map(val => {
                return {
                  title: val.usefulContent
                }
              })
              this.num++
            } else {
          ...
          WXAJAX.POST({
            pageSize: 99999,
            pageNum: 1
          }, '', '/useful/org/ownQueryList', '', '', 1).then((data) => {
            if (data.list) {
          ...
          WXAJAX.POST({
            pageSize: 99999,
            pageNum: 1
          }, '', '/useful/platform/ownQueryList', '', '', 1).then((data) => {
            if (data.list) {
      ...
      uploadFiles(filePath, contentType, duration) {
        if (contentType === 4) wx.showLoading({
          title: '上传中。。',
          mask: true
        })
        return WXAJAX.UploadImage(filePath).then(res => {
          ...
          if (res.code == 200) {
            WebSocket.sendMessage({
              ...
              targetId: this.chatGroupId,
              targetCardId: this.chatGroupId,
              targetCompanyId: this.chatGroupId,
              duration: duration
            })
          } else {
            wx.showToast({
      ...
      getTime() {
        let priceObj = {};
        if (this.msgGroupCont.data) {
          let times = this.msgGroupCont.data.map((res, idx) => {
      ...
      dateF(data) {
        ...
        let m = (new Date(data).getMinutes() - 10) < 0 ? "0" + new Date(data).getMinutes() : new Date(data)
          .getMinutes();
        let s = (new Date(data).getSeconds() - 10) < 0 ? "0" + new Date(data).getSeconds() : new Date(data)
          .getSeconds();
    ...
    watch: {
      msgGroupCont: {
        handler() {
          // this.cardId = this.msgGroupCont.cardId
          // this.companyId = this.msgGroupCont.companyId
          console.log('msgGroupCont111111111', this.msgGroupCont)
          this.getTime();
        },
        deep: true,
      },
    ...
    computed: {
      ...mapGetters(['msgGroupCont', 'currentCompany', 'changePosition']),
    }
  }
</script>
