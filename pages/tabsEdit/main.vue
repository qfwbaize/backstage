<template>
  <div class="h100p pb70" @click="color_hied_tap">
    ...
    <div class="pb54">
      <div class="bgfff mt10 pl17 pr15 fs16 ca8 lh44" v-for="(v,k) in lists" :key="k">
        ...
        <div class="disflex jsbet align-cen">
          ...
          <div class="disflex">
            <img src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/del.png"
                 @click.stop="del(0,k,0)"
                 alt="" class="w15 h15 enlarge-range"
            />
          </div>
        </div>

        <div v-for="(item,index) in v.list" :key="index" class="disflex jsbet posre">
          ...
          <div class="disflex align-cen ">
                    <span class="w15 h15 bradius50p label" :style="{background : item.lableColor}"
                          @click.stop="color_show_tap(this,$event,item.id, item , k ,index)"></span>
            <img @click="del(1,k,index)" src="https://myy-one-stand.oss-cn-beijing.aliyuncs.com/yimai_photos/crm/del_2.png"
                 alt="" class="w16 h16 enlarge-range"
            />
          </div>
          <div class="posab shadow_gray  bradius5 textc choose_color disflex"
              v-if="item.selectColor"
              :style="{top:0 + 'px'}">
            <div class="bgfff color-select bradius5 pb25">
              <b @click.stop="choose_color('#FD634E',item)" class="w15 h15 bradius50p bgorange"></b>
              <b @click.stop="choose_color('#FFA133',item)" class="w15 h15 bradius50p bgyellow"></b>
              <b @click.stop="choose_color('#2BCF88',item)" class="w15 h15 bradius50p bggreen"></b>
              <b @click.stop="choose_color('#51CDCB',item)" class="w15 h15 bradius50p bgblue"></b>
              <b @click.stop="choose_color('#566C84',item)" class="w15 h15 bradius50p bg55"></b>
              <b @click.stop="choose_color('#F5F5F6',item)" class="w15 h15 bradius50p bgf5f6"></b>
            </div>
        ...
        <!--添加子标签-->
        <div class="disflex jsbet" @click.stop="add(1,k)">
          <span>添加子标签</span>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
            ...
            data.forEach(function (i1, k1 ) {
              ...
              i1.lableResponses && i1.lableResponses.forEach(function (i2,k2) {
                datas[k1].list.push({
                  ...
                  lableColor : i2.lableColor || '#F5F5F6',
                  selectColor: false
                })
              })
      ...
      color_show_tap(obj, dom, id  , item , index1 ,index2) {
          this.lists.forEach((v,k)=>{
            for(var i in v.list){
              v.list[i].selectColor = false
            }
          })
        
        item.selectColor = !item.selectColor
        return
        let v = this;
      ...
      add(method,index1){//添加标签
        ...
        if(method == 0){
          ...
          this.lists.push(
            {
              title: '',
              list: [
                { lableName: '', lableColor: '#F5F5F6', selectColor: false},
                { lableName: '', lableColor: '#F5F5F6', selectColor: false},
              ]
            }
          )
          ...
          this.lists[index1].list.push(
            { lableName: '', collableColoror: '#F5F5F6', lableColor: '#F5F5F6',selectColor: false},
          )
        }

      },
      choose_color(color,item){//选择颜色
        item.lableColor = color
