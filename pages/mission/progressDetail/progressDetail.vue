<template>
    ...
    <view class="contract" style="padding-top: 20rpx;" v-for="(item,index) in selfMoney" :key="index" v-show="situationList.length !== 0">
      <view class="contractText">{{item.create_time}}</view>
      <view class="contractText">任务已完成验收 奖励金额{{item.money}}</view>
      <view class="contract_speed">
   ...
   <view class="contract" style="padding-top: 20rpx;" v-for="(item,index) in situationList" :key="index">
    ...
    <view class="task_button" v-if="situationList.length !== 0">
  ...
  import http from "../../../utils/http"
    ...
    onShow(){
      ...
      goCheck(){
        ...
        http({url:'/apis/companytask/task_people_edit',method:'GET',data:data}).then(res=>{
          if(res.data.code = 200){
              uni.showModal({
                  title: '提示',
                  content: '是否跳转到上传奖励页面',
                  success: function (res) {
                      if (res.confirm) {
                          uni.navigateTo({
                            url:'../upLoadImg/upLoadImg?card_id='+this.card_id+'&task_id='+this.task_id
                          })
                      } else if (res.cancel) {
                          console.log('用户点击取消');
                      }
      ...
      getMissionSituation(id){
        ...
        http({url:'/apis/companytask/task_people_evidence',method:'GET',data:data}).then(res=>{
          if(res.data.code = 200){
            ...
            // console.log(this.situationList,333333333)
          }
        })
      },
