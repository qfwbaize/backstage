<template>
    ...
    <div class="disflex jsbet " @click="toEssayType">
      ...
      <div class="disflex align-cen">
        <input type="text" id="industry" class="pha8 textr h49 pr12" placeholder="选择分类" v-model="choose" readonly
               disabled>
      ...
      @ready="onEditorReady">
		...
		<div>
      <div class='toolbar' :style="{position: isAndroid ? 'fixed' : 'fixed', bottom: isAndroid ? '47px' : `${isBottom}px`}"  @click="format">
...
export default {
  ...
  onShow() {
    ...
    getdata(){
      ...
      }, '', '/company/getDynamicDetail').then((data) => {
        if(data){
